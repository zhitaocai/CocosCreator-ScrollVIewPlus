...
@ccclass
export default class LeftItemPrefab extends cc.Component {
    @property(cc.Node)
    ...
    bindData(index: number) {
        ...
        this.picSprite.node.color = cc.color(255, 255 * ((index % 50) / 50), 255, 255);
    }
    ...
    onPositionChange(xOffsetPercent: number, yOffsetPercent: number) {
        this.contentNode.scale = 0.4 + 0.6 * (0.5 - Math.abs(yOffsetPercent - 0.5));
        this.contentNode.setPosition(-55 + 50 * ((0.5 - Math.abs(yOffsetPercent - 0.5)) / 0.5), 0);
    }
}
